...
class TextComponent extends Component {
    ...
    TextComponent(this.text, {required this.style});

    @override
    Size measure(Size maxSize) {
        final lines = text.split('\n');
        final contentWidth = lines.fold(0, (max, line) => line.length > max ? line.length : max);
    ...
    void render(CanvasBuffer buffer, Rect bounds) {
        ...
        for (var line in lines) {
           ...
           for (int i = 0; i < style.rightPadding; i++) {
               buffer.drawChar(x + line.length + i, y, ' ', fg: style.color, bg: style.bgColor);
           }
           y += 1;
        }

        final totalWidth = lines.fold(0, (max, line) => line.length > max ? line.length : max) + style.horizontalPadding;
        final leftX = bounds.x + style.leftMargin;

        for (int i = 0; i < style.topPadding; i++) {
            buffer.drawAt(leftX, bounds.y + style.topMargin + i, ' ' * totalWidth, style);
        }
